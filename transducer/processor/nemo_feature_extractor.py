...
class NemoFeatureExtractor(SequenceFeatureExtractor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if normalize != "per_feature":
            raise ValueError(
                "Only per_feature normalization is supported for this configuration."
            )
    ...
    def _compute_features(self, waveform: np.ndarray, seq_len_time: int):
        ...
        valid = mel[:, :seq_len_frames]
        mean = valid.mean(dim=1, keepdim=True)
        std = valid.std(dim=1, keepdim=True, unbiased=False) + 1e-5
        mel = (mel - mean) / std
...
def _input_example(max_batch: int = 1, max_dim: int = 32000, min_length: int = 200):
...
if __name__ == "__main__":
    ...
    feature_extractor = NemoFeatureExtractor(
    ...
    signals, lengths = _input_example()
    with torch.no_grad():
        test_out = feature_extractor(
            signals.squeeze(0).numpy(), lengths=lengths[:1], return_tensors="pt"
        )["input_features"]

    print("test_out shape:", tuple(test_out.shape))
