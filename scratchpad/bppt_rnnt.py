...
class RNNTScanLossFn(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, enc, pred, labels, enc_lens, label_lens, blank_id: int, joiner: nn.Module
    ):
        """
        ...
        for b in range(B):
            ...
            for t in range(Tb):
                ...
                if Ub > 0:
                    ...
                    label_lp = (
                        logp[:Ub, :].gather(1, next_labels[:, None]).squeeze(1)
                    )  # [Ub]
                else:
                    label_lp = torch.empty((0,), device=device, dtype=enc.dtype)

                # DP to compute alpha_next:
                # alpha_next[u] = logsumexp(
                #   alpha_next[u-1] + label_lp[u-1], alpha[u] + blank_lp[u]
                # )
            ...
            # In standard RNNT, the final probability includes a blank at (Tb, Ub) in some
            # conventions.
            ...
            # We'll use a typical "end with blank after last frame" by applying joiner at
            # last frame:
    ...
    def backward(ctx, grad_out):
        ...
        for b in range(B):
            ...
            # In the "paper-style" memory save, you'd checkpoint only every K and recompute
            # within blocks.
            alphas = [None] * (Tb + 1)
            alpha = torch.full((Ub + 1,), LOG0, device=device, dtype=enc.dtype)
            alpha[0] = 0.0
            ...
            for t in range(Tb):
                ...
                alpha_next[0] = alpha[0] + blank_lp[0]
                for u in range(1, Ub + 1):
                    alpha_next[u] = logsumexp2(
                        alpha[u] + blank_lp[u], alpha_next[u - 1] + label_lp[u - 1]
                    )
                alpha = alpha_next
                alphas[t + 1] = alpha
            ...
            # other beta_next[u<Ub] should be -inf because can't emit labels after time
            # ends (in this simple convention)
            ...
            for t in reversed(range(Tb)):
                ...
                pred_u = (
                    pred[b, : Ub + 1].detach().requires_grad_(True)
                )  # [Ub+1, Dpred]
                ...
                if Ub > 0:
                    grad_logp[:Ub, :].scatter_add_(
                        1, next_labels[:, None], -gamma_label[:, None]
                    )
...
class RNNTScanLoss(nn.Module):
    ...
    def forward(self, enc, pred, labels, enc_lens, label_lens):
        return RNNTScanLossFn.apply(
            enc, pred, labels, enc_lens, label_lens, self.blank_id, self.joiner
        )
