...
from transducer.encoder import wav2vec2bert


def test_feature_projection_shapes():
    """Feature projection should map input feature dim to model hidden size."""
...
def test_encoder_forward_shape():
    """Full Wav2Vec2BertModel returns expected shape."""
    config = Wav2Vec2BertConfig(num_hidden_layers=2, position_embeddings_type="relative_key", layerdrop=0.0)
...
def test_remap_hf_state_dict_fuses_qkv():
    """HF q/k/v attention weights fuse into qkv_proj for each layer."""
